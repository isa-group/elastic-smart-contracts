...
function printHelp() {
  ...
  echo "      - 'deployCCStreet' - deploy the street chaincode on the channel"
...
## Call the script to isntall and instantiate a chaincode on the channel
function deployCCStreet() {

  scripts/deployCCStreet.sh $CHANNEL_NAME $CC_SRC_LANGUAGE $VERSION $CLI_DELAY $MAX_RETRY $VERBOSE
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts log.txt street_network.tar.gz street_network
