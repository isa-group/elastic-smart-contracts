...
function printHelp() {
  ...
  echo "      - 'deployCCESC' - deploy the street chaincode on the channel"
...
# file for creating the network is "docker-compose-base-net.yaml" in the ``docker``
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts log.txt ESC_network.tar.gz ESC_network
...
COMPOSE_FILE_BASE=docker/docker-compose-base-net.yaml
