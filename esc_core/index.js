...
async function harvesterListener() {
    ...
    const listener = await contract.addContractListener((event) => {
        ...
        if (event.type === 'analysis'){
            ...
            if(warmUp){
            ...
            }else if(!warmUp && controlCount >= config.frequencyControlCalculate && config.experimentNumber == 2){
                ...
                controlCount = 0;
                avgExecTime = 0;
            }else if(!warmUp && controlCount >= config.frequencyControlCalculate && config.experimentNumber >= 3){
                avgExecTime += event.execDuration/config.frequencyControlCalculate;
