        ...
        const listener = await contract.addContractListener((event) => {
            ...
            if (event.type === 'analysis'){
                ...
                if(warmUp){
                    ...
                    contract.evaluateTransaction('evaluateHistory', timeData, avgExecTime, maxCalculationTime, minCalculationTime).then((res) => {
                    ...
                    contract.evaluateTransaction('evaluateFrequency', dataFrequency, avgExecTime, maxCalculationTime, minCalculationTime).then((res) => {
                        let newTime = JSON.parse(res.toString());
                        if(newTime < 0.1){
                            newTime = 0.1;
                        }else if (newTime > 60){
